...
class SilveryAppTransform extends Transform {
    private static Project project
    private SilveryTrackExtension extension

    SilveryAppTransform(Project project, SilveryTrackExtension extension) {
    ...
    void _transform(Context context, Collection<TransformInput> inputs, TransformOutputProvider outputProvider, boolean isIncremental) throws IOException, TransformException, InterruptedException {
        ...
        inputs.each { TransformInput input ->
                    ...
                    dir.traverse(type: FileType.FILES, nameFilter: ~/.*\.class/) {
                        File classFile ->
                            if (SensorsAnalyticsClassModifier.isShouldModify(classFile.name)) {
                                ...
                                if (modified != null) {
                                    /**key 为包名 + 类名，如：/com/autotrack/android/app/MainActivity.class*/
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                if (!extension.disableAppClick) {
                    modifiedJar = SilveryTrackClassModifier.modifyJar(jarInput.file, context.getTemporaryDir(), true)
                }
                if (modifiedJar == null) {
